...
if st.session_state.page == "upload":
    ...
    st.info("Generate future sales predictions based on your historical data")
    d = load_data()
    if d is not None and not d.empty:
        c1,c2 = st.columns(2)
        with c1: s = st.date_input("Forecast Start Date", datetime(2009,1,1))
        with c2: e = st.date_input("Forecast End Date", datetime(2050,12,1))
        if st.button("Generate Forecast", type="primary"):
            try:
                m = fit_arima(d["Amount"], (17,2,3))
                f = m.predict(start=pd.to_datetime(s), end=pd.to_datetime(e))
                c1,c2,c3,c4 = st.columns(4)
                with c1: st.metric("Min Forecast", f"{f.min():.2f}")
                with c2: st.metric("Max Forecast", f"{f.max():.2f}")
                with c3: st.metric("Avg Forecast", f"{f.mean():.2f}")
                with c4: st.metric("Growth", f"{((f.iloc[-1]/d['Amount'].iloc[-1])-1)*100:.1f}%")
                st.line_chart(f)
                st.dataframe(f)
                st.download_button("Download Forecast", f.to_csv(), "forecast.csv")
